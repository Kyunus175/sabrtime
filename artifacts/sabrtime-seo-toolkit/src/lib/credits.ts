...
export function checkDailyLimit(): boolean {
  return getUsage().used < DAILY_FREE_LIMIT;
}
...
// These are intentionally UI/demo-only until server-side accounts and billing exist.
export function getCreditBalance(): number {
  return 0;
}

export function canUsePremiumTool(): boolean {
